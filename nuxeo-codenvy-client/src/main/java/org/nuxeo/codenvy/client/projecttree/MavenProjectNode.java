...
public class MavenProjectNode extends JavaProjectNode {
    ...
    public MavenProjectNode(TreeNode<?> parent, ProjectDescriptor data,
            MavenProjectTreeStructure treeStructure,
    ...
    protected static boolean isSourceFolder(ItemReference item) {
        return isFolder(item) && item.getPath().endsWith("src/main/java") ||
                item.getPath().endsWith("src/test/java");
    }
    ...
    protected void getModules(ProjectDescriptor project,
        ...
        final Unmarshallable<Array<ProjectDescriptor>> unmarshaller =
                dtoUnmarshallerFactory.newArrayUnmarshaller(ProjectDescriptor
                        .class);
        projectServiceClient.getModules(project.getPath(),
